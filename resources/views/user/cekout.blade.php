			...
			<div class="row">
				<div class="col-lg-8 order-2 order-lg-1">
					<form class="checkout-form" method="post" action="{{ route('proses_cekout', Auth::user()->id) }}">
						@csrf
						<div class="cf-title">Alamat Pemesanan</div>
						<div class="row address-inputs">
							...
							<div class="col-md-6">
								...
								@else
									<input type="text" name="kabupaten" placeholder="Kota">
								@endif
								@error('kabupaten')
								 	<div class="alert alert-danger">{{ $message }}</div>
								@enderror
						...
						<button class="site-btn submit-order-btn">Lanjutkan Pesanan</button>
					</form>
				</div>
					...
					@if(session('cart'))
						...
						<ul class="product-list">
							...
							<li>
								...
								<span>Jumlah: {{ $details['quantity'] }}</span>
								<h6 style="margin-top: -13px;">Harga : Rp. {{ number_format($details['price'] * $details['quantity']) }}</h6>
							</li>
							@php
						...
						<ul class="price-list">
							...
							<!-- Tambahkan kode ini untuk menampilkan total keseluruhan -->
